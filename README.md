...
- 开头直接给出判断结论
- 用 `看行业`、`看用户`、`看竞争`、`看自己`形成机会依据
- 用 `看机会`收敛出同一个主机会点
- 用 `3定`承接同一个主机会点，形成后续动作
- 明确处理证据边界、假设项与后续补证方向
- 通过反向验证检查结构、证据、判断和表达质量
...
`3定`必须围绕`看机会`中的主机会点展开。若任一章节无法说明服务哪个机会点，文档判定为结构失焦。
...
- 文档标题统一采用 `XXXX_机会洞察`
- 第一部分必须是 `执行摘要`
- 主体必须严格采用 `执行摘要 + 看行业 + 看用户 + 看竞争 + 看自己 + 看机会 + 定控制点 + 定目标 + 定策略 + 附录`
- 不得增加框架外的额外章节
- 结论必须可执行
- 至少包含一张对比表或判断矩阵
- 各级标题之间加入分割线，提升扫描效率
- 用户场景旅程图必须采用横向阶段卡片图，而不是只给一段孤立 Mermaid 代码
...
- 正式判断书、高优先级方向判断、对外汇报稿默认执行最多 `3` 轮反向验证
- 第 `3` 轮后仍无法达到发布条件时，输出失败报告，不输出正式判断书
...
- Mermaid 图应与旅程表逐项对应，并固定展示阶段、发生、核心场景、痛点、期望五类信息
...
- 红军 subagent 只负责挑错，不改写正文
- coordinator 负责裁决异议、修订最终稿并保持单一文风
- 第 `1` 轮检查同题性与收敛性
- 第 `2` 轮检查证据性与判断强度
- 第 `3` 轮检查表达纪律与可执行性
- `3` 轮后仍存在关键结构、证据、判断或表达问题时，正式文档输出失败
